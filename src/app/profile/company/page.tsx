...
export default function Page() {
  const [data, setData] = useState<CompanyProfileDTO | null>(null);
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) {
    ...
    const newValue = name === "rut" ? formatRut(value) : value;
  ...
  function validateForm(): boolean {
    ...
    const userNameError = validators.required(form.userName, "Nombre de usuario") || validators.minLenght(form.userName, 3, "Nombre de usuario") || validators.maxLenght(form.userName, 50, "Nombre de usuario");
    if (userNameError) errors.userName = userNameError;

    const companyNameError = validators.name(form.companyName, "Nombre de la empresa");
    if (companyNameError) errors.companyName = companyNameError;

    const legalNameError = validators.name(form.legalName, "Razon social");
  ...
  async function handleProfilePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowedTypes.includes(file.type)) {
      setPhotoUploadStatus(`Solo se permiten: ${allowedTypes.map(t => t.replace("image/", ".")).join(", ")}`);
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setPhotoUploadStatus(err.response?.data?.message || "Error al subir la foto");
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-indigo-700 via-purple-700 to-fuchsia-700 p-10">
      <div className="max-w-5xl mx-auto space-y-6">
        <h1 className="text-2xl font-bold text-white">Perfil — Empresa</h1>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative w-28 h-28 rounded-full bg-gray-200 flex items-center justify-center text-3xl font-bold overflow-hidden">
              {data?.profilePhoto ? <img src={data.profilePhoto} alt="Foto de perfil" className="w-full h-full object-cover" /> : <span className="text-gray-500">{data?.companyName?.charAt(0).toUpperCase() || "P"}</span>}
            </div>
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <ProfileField label="Nombre de la empresa" name="companyName" value={form.companyName} editing={editing} fieldErrors={fieldErrors} handleChange={handleChange} data={data?.companyName} />
              <ProfileField label="Razon social" name="legalName" value={form.legalName} editing={editing} fieldErrors={fieldErrors} handleChange={handleChange} data={data?.legalName} />
...
function ProfileField({ label, name, value, editing, fieldErrors, handleChange, data, textarea = false, colSpan = 1 }: { label: string; name: string; value: string; editing: boolean; fieldErrors: Record<string, string>; handleChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void; data?: string; textarea?: boolean; colSpan?: number }) {
  return (
    <div className={colSpan > 1 ? `md:col-span-${colSpan}` : ""}>
      <label className="block text-sm font-medium mb-1">{label}</label>
      {editing ? (textarea ? <><Textarea name={name} value={value} onChange={handleChange} rows={6} maxLength={500} className={`resize-y min-h-32 ${fieldErrors[name] ? "border-red-500" : ""}`} placeholder={`Escribe ${label}...`} /> {fieldErrors[name] && <p className="text-red-600 text-xs mt-1">{fieldErrors[name]}</p>}</> : <><Input name={name} value={value} onChange={handleChange} className={fieldErrors[name] ? "border-red-500" : ""} /> {fieldErrors[name] && <p className="text-red-600 text-xs mt-1">{fieldErrors[name]}</p>}</>) : textarea ? <div className="px-3 py-2 border rounded-md bg-gray-50 min-h-[100px] whitespace-pre-wrap">{data || "Sin descripción"}</div> : <div className="px-3 py-2 border rounded-md bg-gray-50">{data || "-"}</div>}
    </div>
  );
}
