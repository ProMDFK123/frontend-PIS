...
export default async function CreatePublicationPage() {
  ...
  console.log("[guard]/students session:", Boolean(session), session);
  // eslint-disable-next-line no-console
  console.log("[guard]/students tokenCookie:", Boolean(tokenCookie), tokenCookie);

  if (!session && !tokenCookie) {
    const returnTo = encodeURIComponent('/students');
