...
export function ProfileSidebarSection({ profile, onPhotoUpload }: ProfileSidebarSectionProps) {
    ...
    const fullName = `${profile.companyName || ""} ${profile.legalName || ""}`.trim();
    const initials = `${profile.companyName?.charAt(0) || ""}${profile.legalName?.charAt(0) || ""}`;
        ...
        <section className="w-full">
            ...
            <div className="mt-6 pt-5 border-t border-slate-200 text-slate-700 text-sm space-y-4">
                ...
                {profile.rating !== undefined && (
                    <div className="flex flex-col">
                        <span className="font-semibold mb-1">Calificación:</span>
                        <div className="flex items-center gap-2">
                            <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                            <span className="font-bold text-slate-900">
                                {profile.rating.toFixed(1)} / 6.0
                            </span>
                        </div>
                    </div>
                )}
