// src/views/app/students/your-publications/[id]/index.tsx
"use client";
import { useMemo, useState } from 'react'; 
import { useRouter, useSearchParams, useParams } from 'next/navigation';
import Link from 'next/link';
// Se añade 'Edit' a la lista de importaciones de lucide-react
import { ArrowLeft, Briefcase, ShoppingBag, Heart, Users, Trash2, Edit } from 'lucide-react'; 
...
const PUBLICATION_TYPES = [
    { value: 0, text: "Oferta de Trabajo", icon: Briefcase, iconClass: "text-indigo-500" },
    { value: 1, text: "Compra/Venta", icon: ShoppingBag, iconClass: "text-purple-500" }, // 1 = Compra/Venta para corregir bug
    { value: 2, text: "Voluntariado", icon: Heart, iconClass: "text-pink-500" }, 
];
const PUBLICATION_STATUS = [
    { value: 0, text: "Activa", classes: "bg-green-500 text-white" },
    { value: 1, text: "Pendiente", classes: "bg-yellow-400 text-slate-900 border border-yellow-500/50" },
    { value: 2, text: "Rechazada", classes: "bg-red-500 text-white" },
];
// --------------------------------------------------------

export default function StudentYourPublicationDetailView() {
    ...
    const handleCloseConfirm = async () => {
        ...
        try {
            ...
            router.push(`/students/your-publications?notification=closed`); 
    ...
    const publicationType = useMemo(() => {
        const typeValue = (publication as any)?.types || (typeQuery ? parseInt(typeQuery) : undefined);
        return PUBLICATION_TYPES.find(t => t.value === typeValue) || { text: "Desconocido", icon: Briefcase, iconClass: "text-gray-400" };
    }, [publication, typeQuery]);
    
    const publicationStatus = useMemo(() => {
        const statusValue = (publication as any)?.statusValidation ?? (statusQuery ? parseInt(statusQuery) : undefined);
        const statusInfo = PUBLICATION_STATUS.find(s => s.value === statusValue) || { text: "Desconocido", classes: "bg-gray-500 text-white" };
    ...
    const isJobOffer = publication && ('remuneration' in publication || 'companyName' in publication);
    const isPending = publication && publication.statusValidation === 1; // Variable ya existente
    const isRejected = publication && publication.statusValidation === 2; // Variable ya existente
    const isPublished = publication && publication.statusValidation === 0;
    
    // Handler para navegación a postulantes
    const handleViewApplicants = () => {
        if (publication) {
            router.push(`/students/your-publications/${publication.id}/applicants`);
        }
    }


    if (isLoading) {
        return <div className="text-center p-20 text-white">Cargando detalles...</div>; 
    }
    ...
    const renderContent = () => {
        ...
        const isActionable = detail.statusValidation === 0 || detail.statusValidation === 1 || detail.statusValidation === 2;
                    ...
                    {isActionable ? (
                        <div className="pt-8 border-t border-slate-100">
                             <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                {/* ** Botón Editar Publicación ** (Añadido) */}
                                {(isPending || isRejected) && (
                                    <Link href={`/students/edit-publication/${detail.id}`} passHref>
                                        <button 
                                            className="flex-1 px-6 py-4 bg-yellow-500 text-slate-900 rounded-xl font-bold hover:bg-yellow-600 transition shadow-lg flex justify-center items-center gap-2 text-lg"
                                            disabled={isMutating}
                                        >
                                ...
                                {/* ** Botón Ver Postulantes ** - Condición: Solo si es Oferta/Voluntariado (isJobOffer) y Publicada */}
        ...
        <div className="flex flex-col min-h-screen relative text-white selection:bg-pink-500 selection:text-white bg-slate-900">
            ...
            <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
                <header className="mb-10">
                    <Link href="/students/your-publications"> 
                    ...
                    <div className="flex flex-col items-start gap-2">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white text-xs font-bold uppercase tracking-wider shadow-lg transform rotate-1">
                            <publicationType.icon className={cn("w-3.5 h-3.5", publicationType.iconClass)} /> 
