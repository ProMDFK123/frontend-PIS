...
export default function StudentYourPublicationDetailView() {
  ...
  const publicationTitle =
    publication?.title ||
    (isLoading ? "Cargando Publicación..." : "Detalle no disponible");
  ...
  const renderContent = () => {
    if (!detail) {
      return (
        <div className="bg-white text-slate-900 rounded-[2.5rem] shadow-2xl p-10 text-center">
            ...
            <div className="pt-8 border-t border-slate-100">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                {(isPending || isRejected) && (
                  <Link
                    href={`/students/edit-publication/${detail.id}`}
                    passHref
                  >
                    <button
                ...
                {/* ** Botón Ver Postulantes ** - Condición: Solo si es Oferta (0) o Voluntariado (2) y Publicada */}
                {isPublished &&
                  (publicationType.value === 0 ||
                    publicationType.value === 2) && (
                    <button
