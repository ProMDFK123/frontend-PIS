...
import { OfferBasicDto, BuySellBasicDto } from "src/services/dtos/dto";
import { OfferForAdmin } from "src/types/admin-publications";
import {
  PendingOffersForAdminDto,
  OfferDetailForAdminDto,
  BuySellDetailForAdminDto,
} from "src/services/dtos/adminDto";
import { AdminDetail, PublicationType } from "src/types/admin-publications";
...
export function mapOfferDtoToCard(o: OfferBasicDto): Offer {
  ...
  const ownerName =
    o.ownerName && o.ownerName.trim() !== "" ? o.ownerName : "UCN";
...
function toOfferTypeForAdmin(
  o: PendingOffersForAdminDto
): OfferForAdmin["type"] {
...
export function mapOfferDtoToValidate(
  o: PendingOffersForAdminDto
): OfferForAdmin {
  return {
    id: String(o.id),
...
export function mapOfferDtoToDetail(dto: any): AdminDetail {
  ...
  const titleValue =
    (dto as OfferDetailForAdminDto).Title ?? dto.title ?? "Sin título";
  const descriptionValue =
    (dto as OfferDetailForAdminDto).Description ??
    dto.description ??
    "No hay descripción disponible.";

  // 🚨 CORRECCIÓN USUARIO (Detalle Oferta): Aseguramos CompanyName
  const rawCompanyName =
    (dto as OfferDetailForAdminDto).CompanyName ?? dto.companyName;
  ...
  const remunerationRaw =
    (dto as OfferDetailForAdminDto).Remuneration ?? dto.remuneration ?? 0;
  const cleanRemuneration = String(remunerationRaw).replace(/[^\d.]/g, "");
  const remunerationValue = parseFloat(cleanRemuneration) || 0;

  // Fechas e imágenes
  const publicationDateValue =
    (dto as OfferDetailForAdminDto).PublicationDate ?? dto.publicationDate;
  const statusValidationValue =
    (dto as OfferDetailForAdminDto).StatusValidation ??
    dto.statusValidation ??
    "Published";
  const activeValue =
    (dto as OfferDetailForAdminDto).Active ?? dto.active ?? false;
  const imagesValue =
    (dto as OfferDetailForAdminDto).Images ?? dto.images ?? [];
...
export function mapBuySellDtoToDetail(dto: any): AdminDetail {
  ...
  const titleValue =
    (dto as BuySellDetailForAdminDto).Title ?? dto.title ?? "Sin título";
  const descriptionValue =
    (dto as BuySellDetailForAdminDto).Description ??
    dto.description ??
    "No hay descripción disponible.";

  // 🚨 CORRECCIÓN USUARIO (Detalle BuySell): Aseguramos UserName
  const rawUserName =
    (dto as BuySellDetailForAdminDto).UserName ?? dto.userName;
  const userNameValue =
    rawUserName && rawUserName.trim() !== "" ? rawUserName : "Usuario UCN";

  const publicationDateValue =
    (dto as BuySellDetailForAdminDto).PublicationDate ??
    dto.publicationDate ??
    undefined;
  const priceValue =
    (dto as BuySellDetailForAdminDto).Price ?? dto.price ?? undefined;
